...
class LessonManager extends React.Component{
    ...
    renderTeacherComment(){
      ...
      const {teacher_comment} = lesson;

      const {comment_relate} = this.state;
      ...
      return(
          ...
          <div>
        {
          ...
          <div style={{textAlign: "right", marginTop: '1rem'}}>
            <Button 
              type="primary"  
              onClick={() => this.props.addTeacherComment(this.state.comment_label_id, this.state.label_type, {
                comment_content: this.state.comment_content,
                lesson_id: lesson.lesson_id,
                comment_label: this.state.comment_label,
                teacher_id: this.props.teacher_id,
                at_userid: this.state.at_userid,
              })}
            >点评</Button>
          </div>
          <List
